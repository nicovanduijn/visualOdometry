...
for i = 1:length(previous_state_keypoints)
    ...
    if previous_state_keypoints(1,i)>(r+1) && previous_state_keypoints(1,i)<size(previous_image,1)-(r+1)...
        ...
        w = keypoint_frame{i};
        TemplateData(i).p = [0 0 0 0 previous_state_keypoints(1,i) previous_state_keypoints(2,i)];
        TemplateData(i).image = previous_image(w(1,1):w(1,2), w(2,1):w(2,2)); % pixels of the template
